...
Bước 1 đã có luồng MVP: phụ đề YouTube → Groq dịch tiếng Việt → Edge TTS giọng `vi-VN-HoaiMyNeural` → phát theo `video.currentTime`. Neon, IndexedDB và video không có phụ đề chưa thuộc bước này. Xem [STATUS.md](./STATUS.md).
...
- Chưa có cache Neon/IndexedDB, tab capture, Whisper hay fallback Web Speech.
