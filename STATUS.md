...
> Chống lặp AssemblyAI 2026-08-07: scheduler không còn `played.clear()` cho mọi sự kiện `seeking`; seek tiến/buffering giữ dấu câu đã phát, chỉ tua ngược thật sự mới mở lại các câu từ vị trí đích. Turn AssemblyAI được dedupe thêm bằng fingerprint 30 giây (kể cả bản finalized gần giống/cumulative), ngoài dedupe `turn_order` và lọc audio feedback. `npm run check`, 9/9 test và production build đạt.
